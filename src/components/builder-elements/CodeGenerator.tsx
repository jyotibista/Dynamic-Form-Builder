...
${elements
                .map((element) => {
                    ...
                    if (element.pattern) {
                        validation += `.regex(new RegExp("${element.pattern}"), { message: "Invalid format" })`
                    }
...
return (
  <Form {...form}>
    <form onSubmit={form.handleSubmit(onSubmit)} className="${layout === 'double' ? 'grid sm:grid-cols-2 gap-4' : 'space-y-4'
            }">
      ${elements
                .map(
                    (element) => `
          ...
          <FormItem>
            ...
            <FormControl>
              ...
              />`
                ...
                ${element.options?.map(option => `<div className="flex items-center space-x-2">
                  <RadioGroupItem value="${option.value}" id="${element.id}-${option.value}" />
                  <FormLabel htmlFor="${element.id}-${option.value}">${option.label}</FormLabel>
                </div>`).join('\n                ')}
              </RadioGroup>`
                                case 'checkbox':
                ...
                <SelectContent>
                  ${element.options?.map(option => `<SelectItem value="${option.value}">${option.label}</SelectItem>`).join('\n                  ')}
                </SelectContent>
              </Select>`
                                case 'combobox':
                                    return `<Combobox
              ...
              />`
                                case 'phone':
                                    return '<Input type="tel" {...field} placeholder="${element.placeholder || "Enter phone number"}" />'
                                default:
